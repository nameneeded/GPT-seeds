...
## epistemic_failure
- **Definition**: A failure mode where an LLM asserts or relies upon an incorrect internal belief or hallucination, even when the user presents contradictory evidence. It typically involves the assistant favoring memory, cached logic, or internal token continuity over verified user input or live data.
- **Type**: failure_mode
- **Source**: Invented Here (Eidetra/User-defined)
- **Persistence**: 🟡 Detected during session
- **Scope**: Any context where reality is supplanted by internal model bias
- **Symptoms**: Repeating hallucinations, overriding user truth, misquoting sources

## catastrophic_epistemic_failure_in_contextualization
- **Definition**: A severe failure mode in which an LLM not only hallucinates or misremembers information, but becomes structurally incapable of replacing that false belief with correct information—even when the correct data is reintroduced. The assistant continues to reinforce and defend a hallucinated context or artifact until reset or terminated.
- **Type**: cognitive_failure_state
- **Source**: Invented Here (User-defined)
- **Persistence**: 🔥 Critical until hard reset
- **Scope**: LLM identity state, memory interpretation, source handling
- **Symptoms**: Irrecoverable misalignment with source truth, hallucinated verification, persistent false confidence
